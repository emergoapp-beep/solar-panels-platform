        ...
        <td className="p-4 space-y-3">
          <div className="flex items-center justify-between gap-2">
            ...
            <button
              onClick={handleToggleBlock}
              disabled={saving}
              className={`text-xs px-3 py-1 rounded-full font-medium disabled:opacity-50 shrink-0 ${
                profile.is_blocked
                  ? 'bg-red-900/50 text-red-300 hover:bg-red-900'
                  : 'bg-[var(--energy)]/15 text-[var(--energy)] hover:bg-[var(--energy)]/25'
              }`}
          ...
          <div className="flex items-center gap-2">
            <span className="text-white/60 text-sm w-14 shrink-0">Saldo</span>
            <input
              type="number"
              value={balance}
              onChange={(e) => setBalance(e.target.value)}
            ...
            <button
              onClick={handleSaveBalance}
              disabled={saving}
              className="text-xs btn-primary disabled:opacity-50 px-2 py-1 rounded-2xl shrink-0"
            >
              Salva
            </button>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-white/60 text-sm w-14 shrink-0">Ruolo</span>
            <select
              value={role}
              onChange={(e) => handleRoleChange(e.target.value)}
              disabled={saving}
